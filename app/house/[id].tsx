    ...
    <div style={{ width: '100vw', minHeight: '100vh', background: '#1a1a2e', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 20px 100px' }}>
        {house.photos.length > 0 && (
          <div style={{ marginTop: 20 }}>
            <div style={{ borderRadius: 16, overflow: 'hidden', position: 'relative' }}>
              <img src={house.photos[activePhoto]} alt={house.address} style={{ width: '100%', height: 320, objectFit: 'cover', display: 'block' }} />
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
            {features.map((f: Feature) => (
              <span key={f} style={{ padding: '6px 14px', borderRadius: 20, background: '#2a2a4e', border: '1px solid #444', color: '#FFD700', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
                {FEATURE_EMOJI[f] || '✨'} {f}
              </span>
            ))}
          </div>
          {house.description && <p style={{ color: '#ccc', fontSize: 15, lineHeight: 1.6, margin: '0 0 24px' }}>{house.description}</p>}
        </div>

        {/* Map */}
        <div style={{ marginBottom: 32 }}>
          <h3 style={{ color: '#fff', fontSize: 16, fontWeight: 700, margin: '0 0 12px' }}>📍 Location</h3>
          {mapReady && mapMod && leaflet ? (() => {
            const { MapContainer, TileLayer, Marker } = mapMod;
            const icon = leaflet.divIcon({ className: 'twinkle-marker', html: '<span style="font-size:28px">🎄</span>', iconSize: [28, 28], iconAnchor: [14, 28] });
            return (
          ...
          <a href={`https://www.google.com/maps/dir/?api=1&destination=${house.lat},${house.lng}`} target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-block', marginTop: 10, padding: '8px 16px', borderRadius: 8, background: '#2a2a4e', border: '1px solid #444', color: '#fff', fontSize: 13, textDecoration: 'none', fontWeight: 600 }}>
            🗺️ Get Directions
          </a>
        </div>
        ...
        <div style={{ background: '#2a2a4e', borderRadius: 16, padding: 24, marginBottom: 32, border: '1px solid #333' }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 16, flexWrap: 'wrap' }}>
            {[1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5].map(s => (
              <button key={s} onClick={() => setUserRating(s)} style={{
                width: s % 1 === 0 ? 36 : 24, height: 36, borderRadius: 8,
                ...
                fontSize: s % 1 === 0 ? 14 : 11, fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s',
              }}>{s}</button>
            ))}
          </div>
          <textarea value={reviewText} onChange={e => setReviewText(e.target.value)} placeholder="Share your experience..." rows={3}
            style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: '1px solid #444', backgroundColor: '#1a1a2e', color: '#fff', fontSize: 14, fontFamily: 'inherit', boxSizing: 'border-box', resize: 'vertical', minHeight: 70 }} />
          ...
          {showReviewSuccess && <p style={{ color: '#4ade80', fontSize: 13, margin: '8px 0 0' }}>✓ Review submitted!</p>}
        </div>
            ...
            <div key={r.id} style={{ background: '#2a2a4e', borderRadius: 12, padding: 16, marginBottom: 12, border: '1px solid #333' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 32, height: 32, borderRadius: 16, background: '#444', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ccc', fontSize: 14, fontWeight: 700 }}>{r.user[0]}</div>
